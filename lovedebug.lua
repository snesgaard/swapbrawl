...
_Debug.hotSwapLoad = function()
	local loadok,err=xpcall(
        love.load,_Debug.handleError,
        love.arg.parseGameArguments(arg), arg
    )
	if loadok then
...
_Debug.liveCheckLastModified = function(table1,table2)
    local function check_if_changed(name, ref)
        local info = love.filesystem.getInfo(table1)
        ...
        if type(ref) == "number" then
            info.modtime = info.modtime or ref
            return ref ~= info.modtime
        end
	...
	if type(table1) == 'string' then
        return check_if_changed(table1, table2)
	...
	for i,v in ipairs(table1) do
        ...
        if check_if_changed(v, table2[i]) then
			return true
		end
	end
	return false
			...
			if love.update and not _Debug.drawOverlay then
				if _DebugSettings.LiveAuto and _Debug.liveCheckLastModified(_DebugSettings.LiveFile,_Debug.liveLastModified) then
					if type(_DebugSettings.LiveFile) == 'table' then
                        ...
                        print(_Debug.liveCheckLastModified(_DebugSettings.LiveFile,_Debug.liveLastModified))
						_Debug.hotSwapUpdate(dt,_DebugSettings.LiveFile)
						_Debug.liveLastModified = love.filesystem.getInfo(_DebugSettings.LiveFile).modtime
                        print(_DebugSettings.LiveFile, _Debug.liveLastModified)
						if _DebugSettings.LiveReset then
							_Debug.hotSwapLoad()
						end
					end
